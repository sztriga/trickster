...
# ---------------------------------------------------------------------------
# MCTS-aware evaluation (policy player uses MCTS + model for move selection)
# ---------------------------------------------------------------------------

def _eval_range_vs_random_mcts(
    policy: TrainedPolicy, *, start_g: int, games: int, seed: int, mcts_config: MCTSConfig,
) -> EvalStats:
    from trickster.games.snapszer.mcts_agent import mcts_choose
    from trickster.games.snapszer.game import close_talon
    ...
    for g in range(start_g, start_g + games):
        ...
        mcts_rng = random.Random(_mix_seed(seed, g, 10))
        rnd_rng = random.Random(_mix_seed(seed, g, 11))
        base_seed = seed + (g // 2)
        st = deal(seed=base_seed, starting_leader=0)
        ...
        while not is_terminal(st):
            ...
            if leader == policy_idx:
                action = mcts_choose(
                    st, None, leader, mcts_config, mcts_rng,
                    lead_model=policy.lead_model, follow_model=policy.follow_model,
                )
                ...
                lead_card = _random_action(rnd_rng, legal_actions(st, leader, None))
            if responder == policy_idx:
                resp_card = mcts_choose(
                    st, lead_card, responder, mcts_config, mcts_rng,
                    lead_model=policy.lead_model, follow_model=policy.follow_model,
                )
            else:
                resp_card = _random_action(rnd_rng, legal_actions(st, responder, lead_card))
...
_WORKER_MCTS_POLICY: Optional[TrainedPolicy] = None
_WORKER_MCTS_SEED: int = 0
_WORKER_MCTS_CONFIG: Optional[object] = None


def _worker_init_vs_random_mcts(policy: TrainedPolicy, seed: int, mcts_config: object) -> None:
    global _WORKER_MCTS_POLICY, _WORKER_MCTS_SEED, _WORKER_MCTS_CONFIG
    _WORKER_MCTS_POLICY = policy
    _WORKER_MCTS_SEED = seed
    _WORKER_MCTS_CONFIG = mcts_config


def _worker_eval_vs_random_mcts(args: tuple[int, int]) -> EvalStats:
    start_g, n_games = args
    assert _WORKER_MCTS_POLICY is not None and _WORKER_MCTS_CONFIG is not None
    return _eval_range_vs_random_mcts(
        _WORKER_MCTS_POLICY, start_g=start_g, games=n_games,
        seed=_WORKER_MCTS_SEED, mcts_config=_WORKER_MCTS_CONFIG,
    )


def evaluate_policy_vs_random_mcts_parallel(
    policy: TrainedPolicy, *, games: int = 200, seed: int = 0,
    workers: int = 2, chunk_games: int = 50, mcts_config: MCTSConfig,
    on_progress: Optional[callable] = None,
) -> EvalStats:
    if workers <= 1 or games <= 0:
        return evaluate_policy_vs_random_mcts(
            policy, games=games, seed=seed, mcts_config=mcts_config, on_progress=on_progress,
        )
    workers = max(1, int(workers))
    chunk_games = max(1, int(chunk_games))
    tasks = _make_tasks(games, chunk_games)
    ...
    stats = EvalStats()
    with ctx.Pool(
        processes=workers, initializer=_worker_init_vs_random_mcts,
        initargs=(policy, seed, mcts_config),
    ) as pool:
        for part in pool.imap_unordered(_worker_eval_vs_random_mcts, tasks):
            _merge_stats(stats, part)
            if on_progress is not None:
                on_progress(stats)
    return stats
...
def evaluate_policy_vs_random_mcts(
    policy: TrainedPolicy, *, games: int = 200, seed: int = 0,
    mcts_config: MCTSConfig, on_progress: Optional[callable] = None,
) -> EvalStats:
    stats = _eval_range_vs_random_mcts(
        policy, start_g=0, games=games, seed=seed, mcts_config=mcts_config,
    )
    if on_progress is not None:
        on_progress(stats)
    return stats
...
# ---------------------------------------------------------------------------
# Head-to-head evaluation with per-side MCTS support
# ---------------------------------------------------------------------------

def _eval_range_head_to_head(
    policy_a: TrainedPolicy, policy_b: TrainedPolicy, *,
    start_g: int, games: int, seed: int,
    mcts_config_a: Optional[MCTSConfig] = None, mcts_config_b: Optional[MCTSConfig] = None,
) -> EvalStats:
    from trickster.games.snapszer.mcts_agent import mcts_choose
    from trickster.games.snapszer.game import close_talon
    ...
    for g in range(start_g, start_g + games):
        if g % 2 == 0:
            policies = (policy_a, policy_b)
            configs = (mcts_config_a, mcts_config_b)
            a_idx = 0
        else:
            policies = (policy_b, policy_a)
            configs = (mcts_config_b, mcts_config_a)
            a_idx = 1

        agents: list[Optional[LearnedAgent]] = [None, None]
        mcts_rngs: list[Optional[random.Random]] = [None, None]
        for i in range(2):
            if configs[i] is None:
                agents[i] = _agent_from_policy(policies[i], _mix_seed(seed, g, i))
            else:
                mcts_rngs[i] = random.Random(_mix_seed(seed, g, i))
        ...
        while not is_terminal(st):
            ...
            if configs[leader] is not None:
                action = mcts_choose(
                    st, None, leader, configs[leader], mcts_rngs[leader],
                    lead_model=policies[leader].lead_model,
                    follow_model=policies[leader].follow_model,
                )
                if action == "close_talon":
                    close_talon(st, leader)
                    action = mcts_choose(
            ...
            if configs[responder] is not None:
                resp_card = mcts_choose(
                    st, lead_card, responder, configs[responder], mcts_rngs[responder],
                    lead_model=policies[responder].lead_model,
                    follow_model=policies[responder].follow_model,
                )
            else:
                resp_card = agents[responder].choose_follow(
                    st.hands[responder], lead_card, legal_actions(st, responder, lead_card),
...
_WORKER_H2H_A: Optional[TrainedPolicy] = None
_WORKER_H2H_B: Optional[TrainedPolicy] = None
_WORKER_H2H_SEED: int = 0
_WORKER_H2H_CFG_A: Optional[MCTSConfig] = None
_WORKER_H2H_CFG_B: Optional[MCTSConfig] = None


def _worker_init_h2h(
    policy_a: TrainedPolicy, policy_b: TrainedPolicy, seed: int,
    mcts_config_a: Optional[MCTSConfig], mcts_config_b: Optional[MCTSConfig],
) -> None:
    global _WORKER_H2H_A, _WORKER_H2H_B, _WORKER_H2H_SEED
    global _WORKER_H2H_CFG_A, _WORKER_H2H_CFG_B
    _WORKER_H2H_A = policy_a
    _WORKER_H2H_B = policy_b
    _WORKER_H2H_SEED = seed
    _WORKER_H2H_CFG_A = mcts_config_a
    _WORKER_H2H_CFG_B = mcts_config_b


def _worker_eval_h2h(args: tuple[int, int]) -> EvalStats:
    start_g, n_games = args
    assert _WORKER_H2H_A is not None and _WORKER_H2H_B is not None
    return _eval_range_head_to_head(
        _WORKER_H2H_A, _WORKER_H2H_B,
        start_g=start_g, games=n_games, seed=_WORKER_H2H_SEED,
        mcts_config_a=_WORKER_H2H_CFG_A, mcts_config_b=_WORKER_H2H_CFG_B,
    )


def evaluate_head_to_head_parallel(
    policy_a: TrainedPolicy, policy_b: TrainedPolicy, *,
    games: int = 1000, seed: int = 0, workers: int = 2, chunk_games: int = 250,
    mcts_config_a: Optional[MCTSConfig] = None, mcts_config_b: Optional[MCTSConfig] = None,
    on_progress: Optional[callable] = None,
) -> EvalStats:
    if mcts_config_a is not None or mcts_config_b is not None:
        chunk_games = min(chunk_games, 50)
    if workers <= 1 or games <= 0:
        return evaluate_head_to_head(
            policy_a, policy_b, games=games, seed=seed,
            mcts_config_a=mcts_config_a, mcts_config_b=mcts_config_b,
    ...
    stats = EvalStats()
    with ctx.Pool(
        processes=workers, initializer=_worker_init_h2h,
        initargs=(policy_a, policy_b, seed, mcts_config_a, mcts_config_b),
    ) as pool:
        for part in pool.imap_unordered(_worker_eval_h2h, tasks):
            _merge_stats(stats, part)
            if on_progress is not None:
                on_progress(stats)
    return stats
...
def evaluate_head_to_head(
    policy_a: TrainedPolicy, policy_b: TrainedPolicy, *,
    games: int = 1000, seed: int = 0,
    mcts_config_a: Optional[MCTSConfig] = None, mcts_config_b: Optional[MCTSConfig] = None,
    on_progress: Optional[callable] = None,
) -> EvalStats:
    stats = _eval_range_head_to_head(
        policy_a, policy_b, start_g=0, games=games, seed=seed,
        mcts_config_a=mcts_config_a, mcts_config_b=mcts_config_b,
    )
    if on_progress is not None:
        on_progress(stats)
